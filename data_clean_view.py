...
# 计算橙子中位价格
price = df1['价格'].groupby(df1['省']).median().round(decimals=2).sort_values(ascending=False)
provinces1 = price.index.tolist()
pricevalue = price.tolist()
...
def pricebar():
    d = (
        Bar()
        .add_xaxis(provinces1)
        .add_yaxis("价格/元", pricevalue)
        .set_global_opts(xaxis_opts=opts.AxisOpts(
            axislabel_opts=opts.LabelOpts(interval=0)),
            title_opts=opts.TitleOpts(title="橙子价格"))
        .render("价格中位柱状图.html")
    )
